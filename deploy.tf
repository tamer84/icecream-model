...
terraform {
  ...
  backend "s3" {
    ...
    key            = "resources/icecream-model/tfstate.tf"
    region         = "eu-central-1"
...
module "cicd" {
  ...
  pipeline_base_configs = {
    "name"        = "icecream-model-${terraform.workspace}",
  ...
  codebuild_build_stage = {
    "project_name"        = "icecream-model-${terraform.workspace}",
    "github_branch"       = local.cicd_branch,
    "github_repo"         = "tamer84/icecream-model",
